...
/**
 * Read a range from a Google Sheet (or local Excel fallback).
 */
async function readRange(spreadsheetId, range) {
  ...
  try {
    ...
    console.warn(`[Google API Fallback]: Falling back to local Excel for range ${range}`);
    return readRangeFromLocal(range);
  }
}

/**
 * Read multiple ranges via batchGet (or local Excel fallback).
 */
async function readMultipleRanges(spreadsheetId, ranges) {
  ...
  try {
    ...
    console.warn(`[Google API Batch Fallback]: Falling back to local Excel`);
    const result = {};
    ranges.forEach(r => {
      result[r] = readRangeFromLocal(r);
    });
 ...
 * @returns {Promise<Object<string, string[][]>>} Object mapping range strings to row arrays
 */
async function readMultipleRanges(spreadsheetId, ranges) {
  const sheets = getSheetsClient();
  const response = await sheets.spreadsheets.values.batchGet({
