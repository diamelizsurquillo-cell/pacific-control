...
const fs = require('fs');
const { normalizarMes } = require('./data-maps');
...
function parseOrdenes(rows) {
  ...
  // Detect header indices dynamically from row 0
  const headerRow = rows[0] || [];
  const findCol = (terms, defaultIdx) => {
    const idx = headerRow.findIndex(cell => {
      if (!cell || typeof cell !== 'string') return false;
      const clean = cell.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
      return terms.some(t => clean.includes(t));
    });
    return idx !== -1 ? idx : defaultIdx;
  };

  const colCotizacion = findCol(['cotizacion', 'cotiz'], 0);
  const colActa = findCol(['acta'], 1);
  const colAnio = findCol(['ano', 'anio'], 2);
  const colLugar = findCol(['lugar', 'sede'], 3);
  const colInspeccion = findCol(['nro inspeccion', 'n° inspeccion', 'nro. inspeccion', 'inspeccion'], 4);
  const colProducto = findCol(['codigo de producto', 'cod producto', 'producto'], 5);
  const colTipo = findCol(['tipo de inspeccion', 'tipo'], 6);
  const colInspector = findCol(['inspector'], 7);
  const colCliente = findCol(['cliente'], 8);
  const colDescripcion = findCol(['descripcion', 'detalle'], 9);
  const colFechaInspeccion = findCol(['fecha de inspeccion', 'fecha de muestreo', 'fecha inspeccion'], 10);
  const colObs = findCol(['observaciones', 'observacion', 'acreditacion', 'acreditado', 'alcance'], 11);
  const colFechaTentativa = findCol(['fecha tentativa', 'tentativa'], 17);
  const colFechaLab = findCol(['laboratorio', 'ingreso a laboratorio'], 18);
  const colFechaCert = findCol(['certificado', 'informe de ensayos'], 20);
  const colCodDoc = findCol(['codigo de documento', 'cod documento', 'documento'], 22);
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const cotizacion = String(row[colCotizacion] || '').trim();
    const rawActa = row[colActa];
    const rawCliente = row[colCliente];
    const nroInspeccion = row[colInspeccion] || null;
    ...
    const rawObs = String(row[colObs] || '').trim();
    const rawTipo = String(row[colTipo] || '').trim();
    const rawDesc = String(row[colDescripcion] || '').trim();

    // Determine Acreditacion from Observaciones or fallback fields
    let acreditacion = 'NO ESPECIFICADO';
    const textToCheck = `${rawObs} ${rawTipo} ${rawDesc}`.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');

    if (textToCheck.includes('NO ACREDITAD') || textToCheck.includes('NO-ACREDITAD') || textToCheck.includes('SIN ACREDITAR') || textToCheck.includes('NO ACRED')) {
      acreditacion = 'NO ACREDITADO';
    } else if (textToCheck.includes('ACREDITAD') || textToCheck.includes('ACRED.') || textToCheck.includes('BAJO ALCANCE')) {
      acreditacion = 'ACREDITADO';
    } else if (textToCheck.includes('CANCELAD') || textToCheck.includes('ANULAD')) {
      acreditacion = 'CANCELADO';
    } else if (textToCheck.includes('TESTIFICACION')) {
      acreditacion = 'TESTIFICACIÓN';
    } else if (textToCheck.includes('REPROGRAMAD')) {
      acreditacion = 'REPROGRAMADO';
    } else if (textToCheck.includes('AMPLIACION')) {
      acreditacion = 'AMPLIACIÓN';
    } else if (rawObs && rawObs !== '-' && rawObs !== '—' && rawObs !== 'N/A' && isNaN(Number(rawObs))) {
      acreditacion = rawObs;
    }
    ...
    results.push({
      cotizacion,
      nroActa,
      anio: row[colAnio] || null,
      lugarCodigo: String(row[colLugar] || '').trim(),
      nroInspeccion,
      codigoProducto: String(row[colProducto] || '').trim(),
      tipoInspeccion: rawTipo,
      nroInspector: String(row[colInspector] || '').trim(),
      cliente,
      descripcion: rawDesc, // Texto libre sin diccionario
      fechaInspeccion: excelDateToISO(row[colFechaInspeccion]),
      fechaInspeccionRaw: row[colFechaInspeccion] || null,
      observaciones: rawObs,
      acreditacion,
      fechaTentativa: excelDateToISO(row[colFechaTentativa]),
      fechaLaboratorio: excelDateToISO(row[colFechaLab]),
      fechaCertificado: excelDateToISO(row[colFechaCert]),
      codigoDocumento: String(row[colCodDoc] || '').trim(),
    });
  }
  return results;
}
...
function parseGastos(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    results.push({
      ...
      mesRequerido: normalizarMes(String(row[7] || '')),
